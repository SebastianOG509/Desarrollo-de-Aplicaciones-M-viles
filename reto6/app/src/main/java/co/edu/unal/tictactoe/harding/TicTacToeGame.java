...
public class TicTacToeGame {
    ...
    public boolean isComputerTurn() {
        ...
        for (char c : mBoard) {
            if (c == HUMAN_PLAYER) humanCount++;
            else if (c == COMPUTER_PLAYER) computerCount++;
        }
    ...
    private int getRandomMove() {
        ...
        for (int i = 0; i < mBoard.length; i++) {
            if (mBoard[i] == OPEN_SPOT) availableMoves.add(i);
        }
        if (!availableMoves.isEmpty()) {
            return availableMoves.get(mRand.nextInt(availableMoves.size()));
        }
        return -1;
    }
    ...
    private int getBlockingMove() {
        for (int i = 0; i < mBoard.length; i++) {
            if (mBoard[i] == OPEN_SPOT) {
                ...
                if (checkForWinner() == 2) {  // Si humano ganaría
                    mBoard[i] = OPEN_SPOT;
                    return i;
                }
                mBoard[i] = OPEN_SPOT;
    ...
    private int getWinningMove() {
        for (int i = 0; i < mBoard.length; i++) {
            if (mBoard[i] == OPEN_SPOT) {
                ...
                if (checkForWinner() == 3) {  // Si computadora ganaría
                    mBoard[i] = OPEN_SPOT;
                    return i;
                }
                mBoard[i] = OPEN_SPOT;
    ...
    public int checkForWinner() {
        ...
        for (int i = 0; i < BOARD_SIZE; i++) {
            if (mBoard[i] == OPEN_SPOT) return 0;  // Juego no terminado
        }
    ...
    public char getBoardOccupant(int index) {
        if (index >= 0 && index < BOARD_SIZE) return mBoard[index];
        return OPEN_SPOT;
    }

    // ===============================
    // NUEVOS MÉTODOS PARA GUARDAR ESTADO
    // ===============================
    public char[] getBoardState() {
        return mBoard.clone();  // Devuelve copia del tablero
    }

    public void setBoardState(char[] state) {
        if (state != null && state.length == BOARD_SIZE) {
            mBoard = state.clone(); // Restaura el tablero
        }
    }
}
